...
# --- CRYPTO & LOGIN LOGIC (FIX NAMEERROR) ---
...
async def start_the_grind(key, iv, region):
    print(f"✅ [GRIND] Started | Server: {region.upper()}")
    while True:
        try:
            ...
            if online_writer: online_writer.write(p2); await online_writer.drain()
...
async def MaiiiinE():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Step Major Login (FIXED: Fungsi sudah didefinisikan)
    ...
    # Pastiin xAuThSTarTuP sudah ada di xDL atau didefinisikan
    ...
    print(f"🚀 Bot Online: {auth_res.account_uid}")
